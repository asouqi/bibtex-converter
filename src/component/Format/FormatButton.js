...
const FormatIcon = ({color}) => {
    return <svg className="me-2" viewBox="0 0 24 24" fill="none" width={24} height={24} xmlns="http://www.w3.org/2000/svg">
        <path
...
export const FormatButton = ({format}) => {
    return(
        <label className={`${format} format-button d-flex align-items-center  fs-6 mb-3`} htmlFor={format}>
            <FormatIcon color={FormatEncoder[format].color}/>
            {FormatEncoder[format].text}
        </label>
    )
}

export const FormatButtonWithTip = ({format, message}) => {
    return(
        <label className={`${format} format-button d-flex align-items-center  fs-6 mb-3`} htmlFor={format} data-toggle="tooltip" data-placement="top" title={message}>
            <FormatIcon color={FormatEncoder[format].color}/>
            {FormatEncoder[format].text}
        </label>
    )
